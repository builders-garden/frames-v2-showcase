...
const tipsAndTricksData = [
  {
    id: getTitleId(MenuLink.TipsAndTricksNgrok),
    title: "1) Use ngrok to test your frames v2 app on Warpcast",
    text: (
      <VStack gap="1rem" mb={{ base: 2, md: 4 }} py={2} align="left">
        <Paragraph py={2}>
          You can use <Code>ngrok</Code> to test your new frame directly on your
          phone on Warpcast app.
          <br />
  ...
  {
    id: getTitleId(MenuLink.TipsAndTricksLocalFramesDebugger),
    ...
    text: (
      ...
      >
        <Paragraph>
          You can now generate a domain manifest for localhost directly from the
          debugger, making it easier to test frames locally.
  ...
  {
    id: getTitleId(MenuLink.TipsAndTricksRememberToLoadContext),
    ...
    text: (
      ...
      >
        ...
        />
        <Paragraph>
          If you are locked on this screen, you'll see a request to your server
          but nothing will load until we signal to Warpcast that our app is{" "}
          <Code>ready()</Code>.
        </Paragraph>
      </VStack>
    ),
  ...
  {
    id: getTitleId(MenuLink.TipsAndTricksEveryWebAppCanBeAFrame),
  ...
  {
    id: getTitleId(MenuLink.TipsAndTricksWhyIsMyFrameCached),
        ...
        <ListRoot as="ol" gap={4}>
          ...
          <ListItem>
            ...
            <ListRoot ml={4}>
              ...
              <ListItem>
                Bust by clearing browser cache, on mobile launching it from the{" "}
                <Code>Developers &gt; Frames</Code> will clear WebView cache
              </ListItem>
            </ListRoot>
          </ListItem>
          <ListItem>
          ...
          <ListItem>
            ...
            <ListRoot ml={4}>
              <ListItem>
                Cached based on the cache-control header by your browser as well
                as Warpcast proxy.
              </ListItem>
  ...
  return (
    <VStack justify="center" align="left" w="full" gap={"1.3rem"}>
      ...
      <VStack gap={"1.5rem"} align="left" w="full" listStyleType="decimal">
        {tipsAndTricksData.map((tipAndTrick) => (
          <Box key={tipAndTrick.id}>
            <Subtitle color="primary" id={tipAndTrick.id}>
              {tipAndTrick.title}
            </Subtitle>
            {tipAndTrick.text}
            {tipAndTrick.link ? (
            ...
            {tipsAndTricksData[tipsAndTricksData.length - 1].id !==
            tipAndTrick.id ? (
              <Separator
                mt={{ base: 4, md: "2.5rem" }}
                mb={{ base: 0, md: "1rem" }}
